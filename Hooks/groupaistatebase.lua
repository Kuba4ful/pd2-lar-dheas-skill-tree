...
function GroupAIStateBase:convert_hostage_to_criminal(unit, peer_unit)
	...
	-- local max_minions = 1
	self:convert_hostage_to_criminal(unit, peer_unit)
	...
	-- if peer_unit then
		...
		-- if peer_unit:base():upgrade_value("player", "denis_on_convert_bonus") == not nil then
			-- log('[has denis skill')
	...
	-- else
		...
		-- if managers.player:upgrade_value("player", "denis_on_convert_bonus") == not nil then
			-- log('[has denis skill')
